...
class ItemsAPITest(APITransactionTestCase):
    ...
    def test_item_details(self):
        ...
        self.assertEquals(response.data['item'], {'user id': item.user.id,
                                                  'item name': item.name,
                                                  'price': item.price})
        self.assertEquals(response.data['receipt_details'], {'receipt id': item.receipt.id,
                                                             'merchant name': item.receipt.merchant.name,
                                                             'scan date': item.receipt.scan_date})
        self.assertEquals(response.data['category_details'], {'category id': item.category_id.id,
